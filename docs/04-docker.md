...
попробуй запустить его на другом компьютере -- и начнутся
проблемы.

У тебя Node.js версии 20, а на сервере -- версии 18.
У тебя Windows, а сервер на Linux. У тебя установлены
одни библиотеки, а у коллеги -- другие. Результат?
Классическая фраза разработчиков: **"У меня работает!"** --
а у других нет.

Представь, что ты переезжаешь. Можно перевозить вещи
россыпью -- что-то потеряется, что-то сломается,
что-то забудешь. Розетки в новой квартире другие,
мебель не входит в дверной проём. Хаос. А можно
аккуратно упаковать всё в контейнер: вещи, адаптеры,
инструкции по сборке. Приехал -- открыл -- всё на месте.

## Решение

Docker упаковывает твой сайт вместе со всем
необходимым (Node.js, библиотеки, настройки)
в контейнер, который работает одинаково везде:
на твоём компьютере, на сервере, на компьютере друга.

```
Твой код + Node.js + библиотеки → Docker-контейнер → Работает везде одинаково
```
...
терминал (в Cursor: **Ctrl+`**) и набери:
...
2. Сайт определит твою систему. Нажми кнопку **"Download"**
...
5. Если спросит про WSL2 -- оставь галочку включённой
   (об этом ниже)
6. Нажми **"Ok"** → дождись → **перезагрузи компьютер**
7. После перезагрузки найди **Docker Desktop** в меню
   "Пуск" и запусти его

**macOS:**

4. Открой скачанный `.dmg` файл
5. Перетащи **Docker** в папку **Программы** (Applications)
6. Запусти Docker из Launchpad или папки Программы
7. macOS может попросить разрешение -- нажми **"Открыть"**

**Проверка (обе системы):**

Подожди пока Docker загрузится -- иконка кита в трее
(Windows) или в строке меню (macOS) должна стать
стабильной. Затем проверь в терминале:
...
> **Windows: Docker Desktop требует WSL2**
>
...
> Обычно установщик сам предлагает поставить WSL2.
> Если спрашивает -- соглашайся. При проблемах --
> попроси AI-помощника помочь.

> **Docker Desktop «тяжёлый»**
>
> Docker Desktop может занимать 2-4 ГБ оперативной
> памяти. Это нормально. Если компьютер начинает
> тормозить, можно закрыть Docker Desktop когда он
> не нужен (правый клик на иконку кита → Quit).
...
Открой терминал в Cursor (**Ctrl+`**), убедись
...
>   с `package.json`.
> - **Ошибки с правами доступа** -- Windows: запусти
>   терминал от имени администратора. macOS: добавь
>   `sudo` перед командой.
...
> и покажи AI-помощнику. Он разберётся.

---

## Попробуй сам

1. **Посмотри контейнеры:** набери `docker ps`
   в терминале -- увидишь список запущенных контейнеров
   с именами и портами
2. **Останови и запусти:** нажми Ctrl+C, потом
   `docker compose up` (без --build) -- заметь,
   второй раз запускается быстрее
3. **Посмотри образы:** набери `docker images` --
   увидишь созданный образ и его размер. Сравни
   с размером папки `node_modules` -- контейнер
   гораздо компактнее!
4. **Измени код и пересобери:** попроси AI поменять
   что-то на сайте, потом выполни
   `docker compose up --build` -- убедись что
...
| Было | Стало |
|------|-------|
| "У меня работает, а у тебя нет" | Работает одинаково везде |
| Установка зависимостей вручную | Всё внутри контейнера |
| Нельзя перенести на сервер просто | `docker compose up` |
